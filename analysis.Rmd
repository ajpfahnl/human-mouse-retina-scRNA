...
# Integration
```{r}
ret.anchors <- FindIntegrationAnchors(object.list = ret.list, dims = 1:20) # anchor.features = f.names, k.filter = NA
...
markers <- FindConservedMarkers(ret.combined, ident.1 = cluster, grouping.var = "orig.ident", verbose = FALSE)
head(markers)
```
```{r}
marker.list <- as.list(c(0:8))
marker.list <- lapply(X = marker.list, FUN = function(x) {
  markers <- FindConservedMarkers(ret.combined, ident.1 = x, grouping.var = "orig.ident", verbose)
})
```

```{r}
FeaturePlot(ret.combined, features = rownames(markers)[1:6], min.cutoff = "q9")
```
